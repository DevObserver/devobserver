...
import { feedCard, feedCardViewed, image, imageWrapper, link } from './FeedCardHero.css';
...
export const FeedCardHero: React.FC<FeedCardSmallProps> = ({
	feed,
	variant = 'small',
	className,
	...
	return (
		<Flex className={`${feedCard} ${feed.viewed ? feedCardViewed : ''} ${className}`} flexDirection="column">
			<NativeLink href={url} rel="noopener" target="_blank" className={link}>
			...
			{/* <Flex className={textWrapper} flexDirection="column" gap={24}>
				<Flex flexDirection="column" gap={8} pr={variant !== 'small' ? 12 : 0} flexGrow={1}>
